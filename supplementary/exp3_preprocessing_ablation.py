...
def patch_interp_method(method):
    ...
    _kind = method  # 给闭包用的别名，避免参数名冲突

    def patched(ts, arr, max_gap_seconds=300, method="linear", min_valid_ratio=0.5, **kwargs):
        # 注意：参数名必须叫 method，与 data_loader.py 中调用方关键字一致；
        # 这里我们忽略外部传入的 method，强制使用闭包变量 _kind（外层 patch_interp_method 的 method）。
        ...
        t_valid = ts[valid]
        arr_valid = arr[valid]
        kind = _kind  # 闭包：用外层 patch_interp_method 设定的 method 覆盖
        for j in range(4):
            col = arr[:, j].copy()
